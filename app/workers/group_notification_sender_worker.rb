class GroupNotificationSenderWorker
  ...
  def perform(group_notification_id, user_id)
    group_notification = GroupNotification.find(group_notification_id)

    group_notification.group.customers.each do |customer|
      notification = Notification.create(customer_id: customer.id, message: group_notification.group_message, group_notification_id: group_notification.id, user_id: user_id)
