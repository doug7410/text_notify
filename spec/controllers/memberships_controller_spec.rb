...
describe MembershipsController do
  ...
  describe "DELETE destroy" do
    it "redirects to the group page" do
      tom = Fabricate(:customer, business_owner: bob_business_owner)
      beer_group = Fabricate(:group, business_owner: bob_business_owner)
      membership = Fabricate(:membership, customer: tom, group: beer_group, current_business_owner: bob_business_owner)
    ...
    it "[does not delete the membership if the group or the customer does not belong to the signed in business_owner]" do
      mike_business_owner = Fabricate(:business_owner)
      tom = Fabricate(:customer, business_owner: mike_business_owner)
      beer_group = Fabricate(:group, business_owner: mike_business_owner)
      membership = Fabricate(:membership, customer: tom, group: beer_group, current_business_owner: mike_business_owner)
      delete :destroy, id: membership.id
      expect(Membership.count).to eq(1)
    end


    it "[sets the flash error message if the membership can't be deleted]" do
      mike_business_owner = Fabricate(:business_owner)
      tom = Fabricate(:customer, business_owner: mike_business_owner)
      beer_group = Fabricate(:group, business_owner: mike_business_owner)
      membership = Fabricate(:membership, customer: tom, group: beer_group, current_business_owner: mike_business_owner)
